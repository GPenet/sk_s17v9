...
int G17B::IsValid7pbf(uint64_t bf) {
	...
	if (zh2b[1].IsValid(bf)) {
		...
		for (uint32_t i = 1; i <= zh2gxn.nua; i++) {
			...
			register uint64_t nu = _popcnt64(ua54);
			if (nu < 21) { 
				t54b12.AddB(ua54); t54b12.AddA(ua54); 
				if (op.f4) {
...
// expand 6 to potential valid bands 1+2

#include "go_bs17p1_cpp.h"

//===============================potential valid bands 1+2 and later
